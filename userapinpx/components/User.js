    ...
    return (
        <Card style={styles.card}>
            <CardItem cardBody style={styles.cardItem}>
                <Image source={{ uri: details.picture?.large, width: 150, height: 250 }} style={styles.image} />
            </CardItem>
            ...
            <CardItem style={styles.cardItem}>
                <Text style={styles.text}>{details.location?.city} {details.location?.state} {details.location?.country}</Text>
            </CardItem>
            <CardItem style={styles.cardItem} footer>
                <Text style={{ color: "#FFF" }}>Registered on {moment(details.registered?.date).format('DD-MM-YY')}</Text>
            </CardItem>
        </Card>
    )
}
